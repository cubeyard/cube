...
const reviews = (supervisor as unknown as { prReviews: { inspect: () => Promise<unknown>; verify: () => Promise<unknown> } }).prReviews;
reviews.inspect = async () => ({ text: "saved diff" });
reviews.verify = async () => ({ verified: true });
assert.deepEqual(await supervisor.reviewPrForUserThread("legacy", metadata[0]!.id, { action: "inspect", token: "test", plan: "test", number: 1, section: "patch" }), { text: "saved diff" });
assert.deepEqual(await supervisor.reviewPrForUserThread("legacy", metadata[0]!.id, { action: "verify", token: "test" }), { verified: true });
