...
- cube.git.inspectPrUpdatePlan(repositoryId, token, plan, { number, section: "patch" | "prDiff", page? }) -> { token, plan, number, section, page, nextPage, complete, text, hash, totalBytes }
...
- cube.git.publishPrUpdate(repositoryId, token, plan) -> { verified, number, stack, plan }
...
- cube.git.verifyPrUpdate(repositoryId, token)
  Read-only reconciliation after a timeout, disconnect, or uncertain publication; do not blindly retry or roll back. A failure after push can mean remote already changed. Closed-but-unmerged/queued layers, non-prefix merges, unverified merge results, forks, incomplete native stack metadata, nonlinear descendant history, and conflicts require reconciliation. Stack metadata cannot be locked atomically with Git refs; concurrent membership changes are detected by pre/post checks, not prevented.
- cube.github.read(number, { type: "issue" | "pr", section?, page?, repositoryId? }) -> { url, data, section, page, nextPage, complete, notice }
  Authenticated read from an attached thread repository (including private repositories); repositoryId defaults to the primary. For a user-supplied URL, match it against cube.repositories.list() and pass that repository's id. Unattached repositories cannot be read; no credentials are exposed.
...
- cube.environment.status() -> { setup, resume, directory }
  Returns lifecycle state and bounded tail logs for setup and resume, and the environment directory that holds them: /workspace/.cube, or a folder under /repos when the project keeps its environment in a reference repository (editable in this thread; retrySetup tests local changes without publication).
...
All attached repositories are thread-local, writable and publishable. Use cube.repositories.list() to select the exact repositoryId and work in its checkout (/workspace for primary, /repos/<checkoutName> for references). Git operations target only that repository; changes are not published together atomically. Modify and publish only repositories authorized by the user. The UI Ship action still targets only the primary repository.
...
const cube = Object.freeze({
  ...
  github: Object.freeze({
    read: (number, options = {}) => __call("github.read", { ...__options(options, ["type", "section", "page", "repositoryId"]), number }),
  }),
