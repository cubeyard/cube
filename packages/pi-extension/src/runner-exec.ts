...
export const unsupportedRunnerFiles = async (): Promise<never> => { throw fail("OPERATION_UNSUPPORTED"); };
export class TrustedRunnerSandbox {
  readonly name = "trusted-runner";
  ...
  async exec(command: string, options: SandboxExecOptions): Promise<{ exitCode: number | null; operationId: string }> {
    ...
    if (result.truncated) options.onData(Buffer.from("\n[runner: output truncated at 8192 bytes; redirect large output to a workspace file]\n"));
