# CONTAINER-ONLY FORK of nixpkgs' nixos/modules/virtualisation/incus.nix.
#
# Upstream wires QEMU into the incus service PATHs unconditionally, so a
# headless appliance that only ever runs SYSTEM CONTAINERS still carries
# qemu plus SDL/GTK4/PipeWire/spice-gtk/zenity: ~1.1 GiB of exclusive
# dead weight in a ~2.9 GiB image. There is no upstream switch, so this
# file adds one: `virtualisation.incus.vmSupport`.
#
# A FORK rather than an mkForce on the unit path: the VM bits are spread
# across three units, the EDK2/seabios firmware farm, spice-gtk and the
# AppArmor rules, so filtering one PATH would leave the rest. Drift is
# caught by the hash assertion below, which fails EVALUATION when
# nixpkgs changes the upstream file instead of silently resurrecting
# QEMU or dropping a helper containers need.
#
# Diff vs upstream: the vmSupport option and the guards it controls; the
# minimalPath option (drops storage drivers, migration, OCI and ACME
# tooling a ZFS-only container host never calls); and this comment.
# Nothing else.
{
  config,
  lib,
...
let
  ...
  path =
    ...
    # Optional storage drivers (btrfs/xfs/lvm+thin), live migration
    # (criu), OCI image import (skopeo/umoci), `incus file mount`
    # (sshfs), wireless (iw) and ACME (lego): ~150 MB that a ZFS-only
    # system-container host never executes.
    ++ lib.optionals (!cfg.minimalPath) [
      btrfs-progs
      criu
      iw
      lego
    ...
    # VM-only helpers: qemu itself (which drags in the whole desktop
    # graphics/audio stack), its libexec, firmware/TPM and virtiofsd.
    ++ lib.optionals cfg.vmSupport (
      with pkgs;
      [
  ...
  environment = lib.mkMerge [
    (lib.mkIf cfg.vmSupport { INCUS_EDK2_PATH = ovmf; })
    # The rendered docs (37 MB) exist for the web UI's help links.
    (lib.mkIf (!cfg.minimalPath) { INCUS_DOCUMENTATION = "${cfg.package.doc}/html"; })
    {
...
in
{
  ...
  options = {
    virtualisation.incus = {
      ...
      vmSupport = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = ''
          Whether the daemon should be able to run VIRTUAL MACHINE
          instances. Off drops qemu, its libexec helpers, cdrkit, swtpm,
          virtiofsd, the EDK2/seabios firmware farm, spice-gtk and
          vhost_vsock — roughly 1.1 GiB of closure on a host that only
          runs system containers.

          Does not exist upstream; it is the reason this module is a
          local fork (see the header).
        '';
      };

      minimalPath = lib.mkOption {
        type = lib.types.bool;
        default = false;
        description = ''
          Drop the optional storage drivers (btrfs, xfs, lvm), live
          migration (criu), OCI import (skopeo, umoci), sshfs, iw, ACME
          (lego) and the rendered documentation from the daemon's
          environment. For a ZFS-only host that runs system containers
          and imports unified tarballs, none of it is ever executed.

          Does not exist upstream (fork addition, see the header).
        '';
      };
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      ...
      # Fork drift guard — rebase the fork, re-measure the closure, then
      # update this hash.
      {
        assertion =
          builtins.hashFile "sha256" "${modulesPath}/virtualisation/incus.nix"
          == "75d3f2b4fb947610457a529127e82d8d0202228630b37d7e85b60cafb25bb628";
        message = ''
          nixos/modules/virtualisation/incus.nix changed upstream —
          scripts/vm/base/incus-container-only.nix is a fork of it and
          must be rebased before this image can be trusted.
        '';
      }
    ];
    ...
    ++ lib.optionals cfg.vmSupport [ "vhost_vsock" ]
    ...
    security.apparmor = {
      ...
      ''
      + lib.optionalString cfg.vmSupport ''
        # Support use of VM instance
        mrix ${pkgs.qemu_kvm}/bin/*,
        k ${OVMF2MB.fd}/FV/*.fd,
        k ${pkgs.OVMFFull.fd}/FV/*.fd,
      ''
      + lib.optionalString (cfg.vmSupport && pkgs.stdenv.hostPlatform.isx86_64) ''
