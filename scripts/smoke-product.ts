...
export async function smokeProduct(root: string, config: string, workspace: string) {
  ...
  try {
    ...
    await reader.cancel();
    assert.equal(fs.readFileSync(path.join(workspace, "product-count"), "utf8"), "once");
    ...
    console.log("ok: concurrent followup deduplication, conflicting text, stop, persisted request identity; removed initial/selected models reopen without silent switch");
